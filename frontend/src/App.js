...
import { 
  ...
  Building2, User, Mail, ExternalLink, AlertCircle
} from "lucide-react";
...
const Dashboard = () => {
  ...
  if (loading) {
    return (
      <div className="p-6 md:p-8 space-y-6" data-testid="dashboard-loading">
  ...
  const statCards = [
    { label: "Total Leads", value: stats?.total_leads || 0, icon: Building2, color: "blue" },
    { label: "Qualified Leads", value: stats?.qualified_leads || 0, icon: CheckCircle, color: "emerald" },
    { label: "Meetings Booked", value: stats?.booked_leads || 0, icon: Calendar, color: "purple" },
    { label: "Total Calls", value: stats?.total_calls || 0, icon: PhoneCall, color: "amber" },
  ];

  return (
    <div className="p-6 md:p-8 space-y-6" data-testid="dashboard">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-gray-900" style={{ fontFamily: "'Barlow Condensed', sans-serif" }}>
            Dashboard
          </h1>
          <p className="text-gray-500 mt-1">AI-powered lead generation and qualification</p>
        </div>
        <Button 
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-gray-900 mt-2" data-testid={`stat-${stat.label.toLowerCase().replace(' ', '-')}`}>
                      {stat.value}
                    </p>
                  </div>
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between mb-2">
                  <span className="text-sm font-medium text-gray-700">Lead to Qualified</span>
                  <span className="text-sm font-bold text-gray-900">{stats?.qualification_rate || 0}%</span>
                </div>
                <Progress value={stats?.qualification_rate || 0} className="h-3" />
              </div>
              <div>
                <div className="flex justify-between mb-2">
                  <span className="text-sm font-medium text-gray-700">Qualified to Booked</span>
                  <span className="text-sm font-bold text-gray-900">{stats?.booking_rate || 0}%</span>
                </div>
                <Progress value={stats?.booking_rate || 0} className="h-3" />
              </div>
            </div>
            ...
            <ScrollArea className="h-[280px]">
              {stats?.recent_calls?.length > 0 ? (
                    ...
                    <div key={idx} className="p-4 hover:bg-gray-50 transition-colors">
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                          call.status === 'completed' ? 'bg-emerald-100' : 'bg-gray-100'
                        }`}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-gray-500">
                            {call.duration_seconds}s duration
                          </p>
                        </div>
                ...
                <div className="p-8 text-center">
                  <PhoneOff className="w-12 h-12 text-gray-300 mx-auto" />
                  <p className="text-sm text-gray-500 mt-3">No recent calls</p>
                </div>
              )}
            </ScrollArea>
          </CardContent>
