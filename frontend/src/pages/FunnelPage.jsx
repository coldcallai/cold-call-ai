...
const FunnelPage = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      let leadsData = [], statsData = {}, agentsData = [], campaignsData = [], setupData = { can_make_calls: true };
      
      try { leadsData = (await axios.get(`${API}/leads`, { headers })).data || []; } catch(e) { console.log('leads fetch failed'); }
      try { statsData = (await axios.get(`${API}/dashboard/stats`, { headers })).data || {}; } catch(e) { console.log('stats fetch failed'); }
      try { agentsData = (await axios.get(`${API}/agents`, { headers })).data || []; } catch(e) { console.log('agents fetch failed'); }
      try { campaignsData = (await axios.get(`${API}/campaigns`, { headers })).data || []; } catch(e) { console.log('campaigns fetch failed'); }
      try { setupData = (await axios.get(`${API}/setup/status`, { headers })).data || { can_make_calls: true }; } catch(e) {}
      
      setLeads(Array.isArray(leadsData) ? leadsData : []);
      setStats(statsData);
      setAgents(Array.isArray(agentsData) ? agentsData.filter(a => a.is_active) : []);
      setCampaigns(Array.isArray(campaignsData) ? campaignsData.filter(c => c.status === 'active') : []);
  ...
  const filteredLeads = selectedCampaign 
    ? safeLeads.filter(l => l.campaign_id === selectedCampaign || !l.campaign_id) 
    : safeLeads;
